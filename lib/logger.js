...
const consoleFormatter = (config) => winston.format.combine(
    ...
    winston.format.printf(({ timestamp, level, message, ...meta }) => {
        ...
        return `${timestamp} [${levelColor}${paddedLevel}${resetCode}] [\x1b[38;2;${parseInt(color.substring(1, 3), 16)};${parseInt(color.substring(3, 5), 16)};${parseInt(color.substring(5, 7), 16)}mID: ${shortSessionId}${resetCode}] [${locationStr}]: ${levelColor}${message}${resetCode}`;
    }),
);
...
const fileFormatter = (config) => winston.format.combine(
    ...
    winston.format.printf(({ timestamp, level, message, ...meta }) => {
        ...
        // Create metadata copy without filePath for JSON (since it's already in locationStr)
        const metaForJson = { ...meta };
        if (config.reportPath && metaForJson.filePath) {
            // Keep filePath in JSON for structured data, but also show in location string
        }

        return `${timestamp} [${paddedLevel}] [ID: ${sessionId}] [${locationStr}]: ${message} ${
            Object.keys(metaForJson).length ? JSON.stringify(metaForJson, null, 2) : ''
        }`;
    }),
);
 ...
 * @param {boolean} reportPathEnabled - Whether to capture call site paths
 * @returns {Function} - Wrapped method that captures call site and adds to metadata
 */
function wrapLoggerMethod(originalMethod, reportPathEnabled) {
    ...
    return function(...args) {
        ...
        if (filePath) {
            ...
            if (typeof lastArg === 'object' && lastArg !== null && !Array.isArray(lastArg)) {
                // Add filePath to existing metadata
                args[args.length - 1] = { ...lastArg, filePath };
            } else {
...
(transports);}
    ...
    if (reportPathEnabled) {
        logger.info = wrapLoggerMethod(logger.info.bind(logger), reportPathEnabled);
        logger.warn = wrapLoggerMethod(logger.warn.bind(logger), reportPathEnabled);
        logger.error = wrapLoggerMethod(logger.error.bind(logger), reportPathEnabled);
        logger.debug = wrapLoggerMethod(logger.debug.bind(logger), reportPathEnabled);
        logger.trace = wrapLoggerMethod(logger.trace.bind(logger), reportPathEnabled);
        logger.fatal = wrapLoggerMethod(logger.fatal.bind(logger), reportPathEnabled);
        logger.internal = wrapLoggerMethod(logger.internal.bind(logger), reportPathEnabled);
    }
